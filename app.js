...
<head>
  ...
  <title>المتجر الأخوي - النسخة المتكاملة</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css" />
  <style>
    /* ===== نفس الـ CSS السابق مع تحسينات بسيطة ===== */
    :root {
      --primary: #2a7f6e;
    ...
    * { margin:0; padding:0; box-sizing:border-box; font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif; }
    body { background:var(--bg); color:var(--text); transition: background var(--transition), color var(--transition); padding-bottom:80px; }
    a { color:var(--primary); text-decoration:none; }
    button { cursor:pointer; border:none; outline:none; font-size:0.9rem; border-radius:8px; transition:var(--transition); }
    ...
    #welcomeOverlay .card {
      background:var(--card-bg); padding:40px 30px; border-radius:var(--radius);
      text-align:center; max-width:400px; width:90%; box-shadow:0 20px 60px rgba(0,0,0,0.5);
    }
    #welcomeOverlay h1 { font-size:2.2rem; color:var(--primary); margin-bottom:8px; }
    #welcomeOverlay p { color:var(--text-light); margin-bottom:20px; }
    #enterBtn { background:var(--primary); color:#fff; padding:12px 28px; font-size:1.1rem; border-radius:30px; }
    #enterBtn:hover { background:var(--primary-light); transform:scale(1.03); }

    /* Header */
    .header {
      display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between;
      padding:14px 0; gap:12px; border-bottom:1px solid var(--border); margin-bottom:18px;
    }
    .logo { font-size:1.6rem; font-weight:700; color:var(--primary); }
    .logo i { margin-left:6px; }
    .header-actions { display:flex; align-items:center; gap:10px; flex-wrap:wrap; }
    .header-actions button {
      background:var(--input-bg); color:var(--text); padding:8px 14px;
      border-radius:30px; font-size:0.9rem; display:inline-flex; align-items:center; gap:6px;
    }
    .header-actions .cart-btn { background:var(--primary); color:#fff; position:relative; }
    .cart-btn #cartCount {
      background:var(--secondary); color:#fff; border-radius:50%; padding:0 7px;
      font-size:0.7rem; position:absolute; top:-6px; right:-6px; min-width:20px; text-align:center;
    }
    #userInfo { display:flex; align-items:center; gap:8px; font-size:0.9rem; }
    #userInfo .btn-secondary { background:var(--secondary); color:#fff; padding:4px 12px; border-radius:20px; font-size:0.75rem; }

    /* Promotions Banner */
    #promotionsContainer {
      background:linear-gradient(135deg,#ffecd2,#fcb69f); color:#333; padding:12px 18px;
      border-radius:var(--radius); margin-bottom:18px; display:none; border-right:6px solid var(--secondary);
      font-weight:500;
    }
    body.dark #promotionsContainer { background:linear-gradient(135deg,#2d1b0e,#4a2c1a); color:#f0e6d3; }
    #promotionsContainer .promo-item { display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px; }
    #promotionsContainer .promo-badge { background:var(--secondary); color:#fff; padding:2px 14px; border-radius:30px; font-size:0.75rem; }

    /* Search & Filters */
    .search-bar { display:flex; gap:10px; margin-bottom:20px; flex-wrap:wrap; }
    .search-bar input {
      flex:1; padding:12px 18px; border:1px solid var(--border); border-radius:30px;
      background:var(--input-bg); color:var(--text); font-size:0.95rem; min-width:180px;
    }
    ...
    .cat-chip {
      padding:8px 18px; border-radius:30px; background:var(--input-bg); color:var(--text);
    ...
    /* Product Grid */
    #productsGrid {
      display:grid; grid-template-columns:repeat(auto-fill,minmax(200px,1fr)); gap:18px; margin-bottom:24px;
    }
    .product-card {
      background:var(--card-bg); border-radius:var(--radius); padding:14px; box-shadow:var(--shadow);
      transition:transform 0.2s, box-shadow 0.2s; position:relative; display:flex; flex-direction:column;
      border:1px solid var(--border);
    }
    .product-card:hover { transform:translateY(-4px); box-shadow:0 8px 24px rgba(0,0,0,0.12); }
    .product-card .discount-badge {
      position:absolute; top:10px; left:10px; background:var(--secondary); color:#fff;
      padding:2px 10px; border-radius:20px; font-size:0.7rem; font-weight:bold; z-index:2;
    }
    .product-card .fav-btn {
      position:absolute; top:10px; right:10px; background:rgba(255,255,255,0.8);
      border-radius:50%; width:32px; height:32px; display:flex; align-items:center; justify-content:center;
      font-size:1rem; color:#aaa; transition:0.2s; z-index:2; cursor:pointer; border:none;
    }
    .product-card .fav-btn.active { color:#e0245e; background:rgba(224,36,94,0.15); }
    .product-card .fav-btn:hover { transform:scale(1.15); }
    .product-img {
      width:100%; height:150px; background:var(--input-bg); border-radius:10px;
      overflow:hidden; display:flex; align-items:center; justify-content:center; margin-bottom:10px; position:relative;
    }
    .product-img img { width:100%; height:100%; object-fit:cover; }
    .product-img .no-img-fallback { font-size:2.5rem; color:var(--text-light); }
    .product-title { font-weight:600; font-size:0.95rem; margin-bottom:4px; flex:1; }
    .product-price { font-weight:700; color:var(--primary); font-size:1.1rem; margin:6px 0; }
    .product-price .old-price { text-decoration:line-through; font-weight:400; color:var(--text-light); font-size:0.8rem; margin-left:8px; }
    .share-buttons button { background:none !important; padding:0 !important; font-size:1.2rem; }
    .btn-add-cart {
      background:var(--primary); color:#fff; padding:8px 0; border-radius:30px;
      font-weight:600; margin-top:8px; transition:0.2s; width:100%;
    }
    .btn-add-cart:hover { background:var(--primary-light); }
    .availability-badge {
      display:inline-block; padding:2px 10px; border-radius:20px; font-size:0.7rem;
      font-weight:bold; margin-top:6px; text-align:center;
    }
    .available { background:#28a745; color:#fff; }
    .unavailable { background:#dc3545; color:#fff; }
    .toggle-availability {
      background:var(--input-bg); border:1px solid var(--border); padding:4px 10px;
      border-radius:20px; font-size:0.7rem; margin-top:4px; color:var(--text);
      transition:0.2s; width:100%;
    }
    .toggle-availability:hover { background:var(--primary); color:#fff; }

    /* Load More */
    #loadMoreBtn { display:none; margin:10px auto 30px; padding:12px 30px; background:var(--primary); color:#fff; border-radius:30px; font-size:1rem; }
    ...
    .modal-content {
      background:var(--card-bg); max-width:600px; width:100%; border-radius:var(--radius);
      padding:24px; max-height:90vh; overflow-y:auto; box-shadow:0 20px 60px rgba(0,0,0,0.3);
    }
    .modal-content h2 { margin-bottom:16px; display:flex; justify-content:space-between; align-items:center; }
    .modal-content .close-btn { background:none; font-size:1.6rem; color:var(--text-light); }
    .cart-item {
      display:flex; justify-content:space-between; align-items:center; padding:10px 0;
      border-bottom:1px solid var(--border); gap:8px; flex-wrap:wrap;
    }
    .qty-btn { background:var(--input-bg); padding:4px 12px; border-radius:20px; font-size:1rem; font-weight:bold; color:var(--text); }
    .summary-line { display:flex; justify-content:space-between; padding:6px 0; font-size:0.95rem; border-bottom:1px dashed var(--border); }
    .summary-line.total { font-weight:700; font-size:1.2rem; border-bottom:2px solid var(--primary); padding-top:10px; margin-top:6px; }
    .discount-text { color:var(--secondary); }
    #checkoutForm input, #checkoutForm select {
      width:100%; padding:10px 14px; margin-top:6px; border:1px solid var(--border);
      border-radius:8px; background:var(--input-bg); color:var(--text);
    }
    #checkoutForm .form-group { margin-top:12px; }
    #submitBtn { width:100%; padding:14px; background:var(--primary); color:#fff; font-weight:bold; font-size:1rem; border-radius:30px; margin-top:16px; }
    ...
    /* Admin Promo Modal */
    #adminPromoModal .modal-content { max-width:500px; }
    #adminPromoModal input {
      width:100%; padding:10px; margin:6px 0 12px; border:1px solid var(--border);
      border-radius:8px; background:var(--input-bg); color:var(--text);
    }
    #adminPromoModal .admin-btn { background:var(--primary); color:#fff; padding:10px; border-radius:30px; width:100%; font-weight:bold; }

    /* Toast */
    #customToast {
      position:fixed; bottom:20px; right:20px; left:20px; max-width:380px; margin:0 auto;
      padding:14px 20px; border-radius:12px; background:var(--card-bg);
      box-shadow:0 8px 30px rgba(0,0,0,0.2); display:none; align-items:center; gap:10px;
      z-index:99999; border-right:5px solid var(--primary); color:var(--text);
    }
    #customToast.success { border-right-color:#28a745; }
    #customToast.error { border-right-color:#dc3545; }
    #customToast.info { border-right-color:#17a2b8; }

    @media (max-width:600px) {
      #productsGrid { grid-template-columns:repeat(auto-fill,minmax(150px,1fr)); gap:12px; }
...
<div id="cartModal" class="modal-overlay">
  <div class="modal-content">
    <h2>🛒 سلة المشتريات <button class="close-btn" onclick="window.toggleCartModal()">&times;</button></h2>
...
<div id="adminPromoModal" class="modal-overlay">
  <div class="modal-content">
    <h2>⚙️ إدارة الفعاليات <button class="close-btn" onclick="window.toggleAdminPromo()">&times;</button></h2>
    <form id="promoForm">
      <div class="form-group">
        <label>عنوان العرض</label>
        <input type="text" id="promoTitle" placeholder="مثل: خصم العيد" required />
      </div>
      <div class="form-group">
        <label>وصف العرض</label>
        <input type="text" id="promoDesc" placeholder="مثل: خصم 15% لأول 5 مشترين" required />
      </div>
      <div class="form-group">
...
<div id="infoModal" class="modal-overlay">
  <div class="modal-content">
    <h2>ℹ️ معلومات المتجر <button class="close-btn" onclick="window.toggleInfoModal()">&times;</button></h2>
...
<!-- Toast -->
<div id="customToast"><span id="toastMessage"></span></div>
...
function validatePhone(phone) { return /^[0-9]{7,15}$/.test(phone.trim()); }
...
// ===== نظام المستخدم =====
function getUser() {
  try { const data = localStorage.getItem('alukhowah_user'); return data ? JSON.parse(data) : null; } catch { return null; }
}
...
function showLoginModal() {
  ...
  if (!phone || !validatePhone(phone)) { showToast('رقم هاتف غير صحيح', 'error'); return; }
...
// ===== نظام الدعوة (خصم 10% بدون حد أقصى) =====
...
function listenPromotions() {
  ...
  }, (error) => { console.warn("خطأ في جلب الفعاليات", error); });
}
function renderPromotions() {
  const container = document.getElementById('promotionsContainer');
  if (!container) return;
  if (state.promotions.length === 0) { container.style.display = 'none'; return; }
...
async function consumePromotion(promoId) {
  ...
  } catch (e) { console.warn("فشل تحديث استخدام العرض", e); }
}
async function createPromotion(e) {
  ...
  if (!title || !desc || !discount || !maxUses) { showToast('يرجى ملء جميع الحقول', 'error'); return; }
  try {
    await addDoc(collection(db, "promotions"), {
      title, description: desc, discountPercent: discount, maxUses, usedCount: 0, active: true, createdAt: serverTimestamp()
    });
    showToast('✅ تم إضافة العرض بنجاح', 'success');
    document.getElementById('promoForm').reset();
  } catch (error) { showToast('❌ فشل الإضافة: ' + error.message, 'error'); }
}
...
function addToCart(id) {
  ...
  if (product.available === false) { showToast('⚠️ هذا المنتج غير متوفر حالياً', 'error'); return; }
  if (product.category === 'شحن ألعاب') { redirectToWhatsApp(product); return; }
...
function calculateFinalTotal() {
  ...
  let deliveryCost = 0;
  if (state.deliveryType === 'inside') deliveryCost = 100;
  else deliveryCost = Math.max(1, Number(state.deliveryKm) || 1) * 35;
  const finalTotal = discountedItemsTotal + deliveryCost;
  return { itemsTotal, smartDiscountPercent, smartDiscountAmount, referralDiscount, promoDiscount, discountedItemsTotal, deliveryCost, finalTotal };
}
...
function initCheckoutForm() {
  ...
  form.addEventListener('submit', async function(e) {
    ...
    if (!validatePhone(phone)) { showToast('رقم الهاتف غير صحيح (7-15 رقم)', 'error'); return; }
    if (!validateAddress(address)) { showToast('العنوان قصير جداً (أقل من 5 أحرف)', 'error'); return; }
    ...
    try {
      if (calc.promoDiscount > 0 && state.promotions.length > 0) {
        await consumePromotion(state.promotions[0].id);
      }
      const orderData = {
        phone: String(phone), address: String(address), items: String(itemsSummary),
        ...
        date: new Date().toLocaleString('ar-EG'), createdAt: serverTimestamp(),
...
function displayProducts(items, append = false) {
  ...
  items.forEach(p => {
    ...
    toggleBtn.onclick = (e) => { e.stopPropagation(); window.toggleProductAvailability(p.id, !available); };
...
function getFavorites() {
  try { const data = localStorage.getItem('alukhowah_favs'); return data ? JSON.parse(data) : []; } catch { return []; }
}
...
function toggleDarkMode() {
  ...
  document.querySelectorAll('.dark-toggle i, #darkModeIcon').forEach(icon => {
...
function compressImage(file, maxWidth = 300, quality = 0.7) {
  return new Promise((resolve, reject) => {
    if (!file || !(file instanceof File)) { reject(new Error('ملف غير صالح')); return; }
...
async function uploadImageToImgBB(fileOrInput) {
  ...
  } catch (e) { console.warn("ImgBB فشل، نستخدم Base64", e); }
...
function initMainPage() {
  ...
  if (promoForm) promoForm.addEventListener('submit', createPromotion);
...
// تصدير Firebase للاستخدامات الأخرى (اختياري)
export {
  db, collection, addDoc, onSnapshot, doc, deleteDoc, query, orderBy,
  serverTimestamp, getDocs, writeBatch, limit, startAfter, getCountFromServer,
